...
<head>
    ...
    <style>
        ...
        .header { text-align: center; color: #333; border-bottom: 2px solid #007acc; padding-bottom: 15px; margin-bottom: 20px; }
        .tabs { display: flex; margin-bottom: 20px; }
        .tab { padding: 10px 20px; background: #f8f9fa; border: 1px solid #dee2e6; cursor: pointer; margin-right: 5px; }
        ...
        .info-card { background: #f8f9fa; padding: 15px; border-radius: 5px; border: 1px solid #dee2e6; }
        .data-section { margin: 20px 0; }
        .price-card { background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color: white; padding: 20px; border-radius: 10px; margin: 10px 0; }
        .api-section { margin-top: 30px; padding-top: 20px; border-top: 2px solid #007acc; }
    </style>
</head>
<body>
    <div class="container">
        <div class="header">
            <h1>🏅 黄金价格服务</h1>
            <p>上海黄金交易所 Au99.99 数据服务 & 钉钉推送</p>
        </div>

        <div class="tabs">
            <div class="tab active" onclick="showTab('price')">📊 价格数据</div>
            <div class="tab" onclick="showTab('push')">🔔 钉钉推送</div>
            <div class="tab" onclick="showTab('api')">🔌 API文档</div>
        </div>
        ...
        <div id="price" class="tab-content active">
            <div class="data-section">
                <h3>📈 实时黄金价格</h3>
                <button class="btn" onclick="refreshPrice()">🔄 刷新数据</button>
                <div id="realtime-data" class="price-card">
                    <p>加载中...</p>
                </div>
            </div>
            ...
            <div class="data-section">
                <h3>📊 历史价格数据</h3>
                <input type="number" id="days-input" placeholder="天数" value="5" style="padding: 5px; margin-right: 10px;">
                <button class="btn" onclick="refreshHistory()">🔄 获取历史数据</button>
                <div id="history-data" class="info-card">
                    <p>点击按钮获取历史数据</p>
                </div>
            </div>
        </div>
        ...
        <!-- 钉钉推送标签页 -->
        <div id="push" class="tab-content">
            <div id="service-status" class="status">
                <h3>📊 推送服务状态</h3>
                <p id="status-text">加载中...</p>
            </div>

            <div style="text-align: center; margin: 20px 0;">
                <button class="btn success" onclick="startService()">🚀 启动定时推送</button>
                <button class="btn danger" onclick="stopService()">⏹️ 停止推送</button>
                <button class="btn" onclick="testPush()">🧪 测试推送</button>
                <button class="btn" onclick="manualOpenPush()">📈 手动推送开盘价</button>
                <button class="btn" onclick="manualClosePush()">📉 手动推送收盘价</button>
            </div>
            ...
            <div class="info-grid">
                ...
                <div class="info-card">
                    <h4>📊 推送统计</h4>
                    <p>启动时间: <span id="start-time">-</span></p>
                    <p>推送次数: <span id="push-count">0</span></p>
                    <p>最后推送: <span id="last-push">-</span></p>
                </div>
            </div>
        </div>
        ...
        <div id="api" class="tab-content">
            <div class="api-section">
                ...
                <div class="info-card">
                    <h4>📊 价格数据接口</h4>
                    <p><strong>GET /api/gold/spot_quotations_sge</strong> - 获取实时黄金价格</p>
                    <p><strong>GET /api/gold/spot_hist_sge?days=N</strong> - 获取历史黄金价格</p>
                    <p><strong>GET /api/gold/info</strong> - 获取API信息</p>
                </div>

                <div class="info-card">
                    <h4>🔔 推送服务接口</h4>
                    <p><strong>GET /api/service/status</strong> - 获取服务状态</p>
                    <p><strong>POST /api/service/start</strong> - 启动推送服务</p>
                    <p><strong>POST /api/service/stop</strong> - 停止推送服务</p>
                    <p><strong>POST /api/push/test</strong> - 测试推送</p>
                    <p><strong>POST /api/push/opening</strong> - 推送开盘价</p>
                    <p><strong>POST /api/push/closing</strong> - 推送收盘价</p>
                </div>
...
curl -X POST http://localhost:5080/api/push/test
    ...
    <script>
        ...
        function refreshPrice() {
            fetch('/api/gold/spot_quotations_sge')
                ...
                .then(data => {
                    ...
                    if (data.status === 'success' && data.data.length > 0) {
                        ...
                        container.innerHTML = `
                            <h4>💰 最新价格</h4>
                            <p><strong>品种:</strong> ${latest.品种 || 'Au99.99'}</p>
                            <p><strong>最新价:</strong> ${latest.最新价 || 'N/A'} 元/克</p>
                            <p><strong>涨跌:</strong> ${latest.涨跌 || 'N/A'}</p>
                            <p><strong>涨跌幅:</strong> ${latest.涨跌幅 || 'N/A'}</p>
                ...
                .catch(error => {
                    document.getElementById('realtime-data').innerHTML = '<p>❌ 网络错误</p>';
                });
        }

        function refreshHistory() {
            const days = document.getElementById('days-input').value;
            fetch(`/api/gold/spot_hist_sge?days=${days}`)
                .then(response => response.json())
                .then(data => {
                    const container = document.getElementById('history-data');
                    if (data.status === 'success' && data.data.length > 0) {
                        let html = '<h4>📊 历史价格数据</h4><table border="1" style="width:100%;border-collapse:collapse;"><tr><th>日期</th><th>开盘</th><th>收盘</th><th>最高</th><th>最低</th></tr>';
                        ...
                        container.innerHTML = '<p>❌ 历史数据获取失败</p>';
                    }
                })
                .catch(error => {
                    document.getElementById('history-data').innerHTML = '<p>❌ 网络错误</p>';
                });
        }
            ...
            fetch('/api/service/status')
                ...
                .then(data => {
                    ...
                    if (data.running) {
                        ...
                        statusText.textContent = '✅ 推送服务运行中';
                    } else {
                        statusDiv.className = 'status stopped';
                        statusText.textContent = '⏹️ 推送服务已停止';
                    }

                    document.getElementById('start-time').textContent = data.start_time || '-';
                    document.getElementById('push-count').textContent = data.push_count;
                    document.getElementById('last-push').textContent = data.last_push || '-';
                });
        }
        ...
        function manualOpenPush() {
            fetch('/api/push/opening', { method: 'POST' })
                .then(response => response.json())
                .then(data => {
                    alert(data.message);
                    updateStatus();
        ...
        function manualClosePush() {
            fetch('/api/push/closing', { method: 'POST' })
                .then(response => response.json())
                .then(data => {
                    alert(data.message);
                    updateStatus();
